...
mcmc_chain <- function(df1, df2, n.players, n.predictors, n.chains=3, ...) {
  res <- list(NULL)
  for (i in 1:n.chains) {
    ...
    alpha0 <- rnorm(n.players, X %*% beta_int0, sigma_alpha0)
    gamma0 <- rnorm(n.players, X %*% beta_slope0, sigma_gamma0)
    res[[i]] <- run_mcmc(dat1=df1, dat2=df2, alpha0=alpha0, gamma0=gamma0, sigma_alpha0=sigma_alpha0, sigma_gamma0=sigma_gamma0, beta_int0=beta_int0, beta_slope0=beta_slope0, ...)
  }
  return(res)
}
...
for (i in yrs) {
  ...
  stopifnot(election_test$NumBallots == unique(election_test$NumBallots))
  ...
  fit <- run_mcmc(dat1=election_train, dat2=player_train, reps=repeats)
  alphas <- fit$alphas[(adapt+burnin+1):n.iter,]
  gammas <- fit$gammas[(adapt+burnin+1):n.iter,]
  
  stuff <- sapply(unique(election_test$YoB), function(x) exp(alphas+gammas*x))
  browser()
